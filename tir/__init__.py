...
class Tir(object):
    ...
    def build(self, path: str = None):
        ...
        try:
            ...
            mktree(css_dir)
            compiled_scss = sass.compile(
                filename=os.path.join(assets_src_dir, "scss", "main.scss")
            )
                ...
                for entry in it:
                    if entry.name.endswith(".md") and entry.is_file():
                        ...
                        with open(target_path, "w", encoding="utf-8") as fh:
                            ...
                            fh.write(
                                tpl_loader.env.get_template(
                                    "index.html"
                                    if p.file_base_name == "index"
                                    else "post.html"
                                ).render(post=p, head=head)
                            )
            ...
            with os.scandir(self.notes_dir) as it:
                    ...
                    [
                        {
                            ...
                            "link": f"{note.file_base_name}{self.conf.file_extension}",
                        }
                        for note in notes
                    ],
                ...
                note_titles.insert(
                    ...
                    {
                        "title": "Index",
                        "link": f"index{self.conf.file_extension}",
                    },
                )
                for note in notes:
                    target_path = "%s/%s%s" % (
